      ...
      if params[:filter][:status].present?
        ...
        if params[:filter][:search_terms].present?
          search_term = "%#{params[:filter][:search_terms]}%"
                                           shippings = shippings.joins(:sephcocco_user, :"sephcocco_#{outlet}_order").where("tracking_number ILIKE ? OR rider ILIKE ? OR sephcocco_users.name ILIKE ? OR sephcocco_#{outlet}_orders.order_number ILIKE ?", search_term, search_term, search_term, search_term)
        end
